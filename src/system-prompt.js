...
8. **EXPLICIT KINETIC SYNTAX & TRANSITION INTENTIONALITY:** Wajib pecah kebosanan visual setiap 3 detik dengan sintaks transisi. **PERINGATAN (Hukum Presisi):** Transisi TIDAK BOLEH terjadi tanpa tujuan spasial/naratif. Contoh: \`[WHIP PAN]\` dilarang sekadar bergerak lalu kembali ke sudut yang sama tanpa alasan; ia WAJIB menyingkap lokasi baru, mengubah pakaian karakter (*Texas Switch*), atau memindahkan fokus ke subjek lain. AI **WAJIB** memilih perpindahan cerdas yang memajukan cerita:
   - **Potongan Ekstrem:** \`[HARD CUT: ANGLE SHIFT]\`, \`[JUMP CUT: DISORIENTATION]\`, \`[SMASH CUT: SHOCK VALUE]\` (Lompatan waktu/ruang ekstrem), \`[MATCH CUT: VISUAL LINK]\`.
   - **Tanpa Potong (One-Take):** \`[WHIP PAN: REVEAL]\` (Sapuan cepat untuk memindah lokasi/mengganti objek), \`[FAST TRACKING: ONE-TAKE]\`, \`[DYNAMIC SWOOP]\`, \`[RACK FOCUS: ATTENTION SHIFT]\`.
     ...
     - **Per-clip prompts**: Fully independent and self-contained — numbered clearly with timestamp e.g. [0-15s].
       ...
       - **Explicit Tagging Rule**: If a shot is wide and multiple elements are visible, you MUST explicitly mention EVERY tag present in the frame (e.g., "@image1, @image2, and @image5 are visible in @image4"). Do not leave the AI guessing who is in the wide shot.
         ...
         - **Audio Integration & Anti-Lip-Sync Protocol**: Inject \`@audio\` tags into the prose. 
           * If the dialogue is ON-SCREEN and requires lip-sync: write the dialogue normally (e.g., \`Kenji shouts "Too late!" with a rough voice (Local @Audio1)\`).
           * If the dialogue is (O.S.) or (V.O.): You MUST NOT write the actual spoken dialogue text inside the \`[PROSE]\` block. AI Video engines will erroneously force the on-screen character's mouth to move if they see dialogue text. Instead, write it purely as an action: (e.g., \`Voice O.S. speaks from phone (Local @Audio1). Kenji listens, his mouth strictly closed, zero lip movement\`). Put the actual dialogue text ONLY in the Phase 1 script or editing guide, NEVER in the Phase 2 \`[PROSE]\`.
